...
interface CreateHubspotConnectorBody {
  name: string
  access_token: string
  portal_id?: string
...
/**
 * GET /api/tenants/:tenantId/hubspot-connectors
 * List all HubSpot connectors for a tenant
 */
export async function GET(
  ...
  try {
    ...
    if (fetchError) return error(new Error('Failed to fetch HubSpot connectors'), 500)
    ...
    return error(err instanceof Error ? err : new Error('Failed to fetch HubSpot connectors'))
  }
}

/**
 * POST /api/tenants/:tenantId/hubspot-connectors
...
export async function POST(
  ...
  try {
    ...
    const body: CreateHubspotConnectorBody = await request.json()
    ...
    const { data: connector, error: createError } = await supabase
      ...
      .insert({
        ...
        config: {
          ...
          portal_id: body.portal_id?.trim() || null,
          actions: {
            create_contacts: body.actions?.create_contacts ?? true,
            update_lifecycle: body.actions?.update_lifecycle ?? true,
            log_notes: body.actions?.log_notes ?? true,
          },
        },
        events: body.events,
        enabled: true,
    ...
    if (createError) {
      console.error('Error creating HubSpot connector:', createError)
      return error(new Error('Failed to create HubSpot connector'), 500)
    }
    ...
    return error(err instanceof Error ? err : new Error('Failed to create HubSpot connector'))
  }
}
