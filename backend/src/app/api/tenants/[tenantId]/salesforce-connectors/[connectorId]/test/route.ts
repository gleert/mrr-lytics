...
import { sendTestSalesforce } from '@/lib/salesforce'
import type { SalesforceConnector } from '@/lib/salesforce'
...
/**
 * POST /api/tenants/:tenantId/salesforce-connectors/:connectorId/test
...
export async function POST(
  ...
  try {
    ...
    const result = await sendTestSalesforce(connector as SalesforceConnector)

    if (!result.success) {
      return error(new Error(result.error ?? 'Failed to connect to Salesforce'), 500)
    }

    return success({ message: 'Salesforce connection verified successfully' })
  } catch (err) {
    return error(err instanceof Error ? err : new Error('Failed to test Salesforce connection'))
  }
}
