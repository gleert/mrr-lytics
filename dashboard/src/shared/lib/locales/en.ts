export const en = {
  translation: {
    ...
    revenue: {
      ...
      sectionAllTitle: 'Transactions in Period',
      sectionAllDesc: 'Invoice items within the selected period, with filters and sorting',
      ...
      transactions: {
        ...
        desc: 'Invoice items for the selected period',
    ...
    settings: {
      ...
      // Revenue accounting
      revenueAccounting: 'Revenue invoice statuses',
      revenueAccountingDesc: 'Choose which invoice statuses count toward revenue, MRR and forecasting calculations. At least one status must be selected.',
      revenueStatusesSaved: 'Revenue statuses updated',
      revenueStatusesError: 'Failed to update revenue statuses',
      revenueStatusesAtLeastOne: 'Select at least one status',
      invoiceStatusDesc: {
        Paid: 'Fully paid invoices — primary revenue source',
        Unpaid: 'Issued but not yet paid — for accrual-based reporting',
        PaymentPending: 'Awaiting payment confirmation',
        Cancelled: 'Cancelled invoices — for credit-note (rectificativa) workflows',
        Refunded: 'Refunded invoices',
        Collections: 'Invoices sent to debt collection',
        Draft: 'Draft invoices not yet issued to clients',
      },
      on: 'On',
      off: 'Off',
      // Theme & Language
    ...
    changelog: {
      ...
      entries: {
        ...
        v280: {
          ...
          clientSignupDate: 'Client signup date now displayed under the client ID in the /clients list',
          arpuHint: 'Hint added under the ARPU card: "Average monthly recurring revenue per active client"',
          productsKpisTop: 'Product KPIs and charts moved to the top of the /products page',
          productsMrrCoherence: 'Products MRR + Add-on Services MRR + Domains MRR now adds up to the canonical Total MRR. Removed the double counting of recurring billable items in the products KPI',
          domainsStatusByPeriod: 'On /domains the status breakdown now respects the selected period for non-active states (cancelled in the period, expired in the period, etc.)',
          domainsTldActive: 'The TLD chart in /domains counts active domains only, not historical',
          billableAvgMrr: 'The /billable-items "Avg amount" KPI is now "Avg MRR" (normalised to monthly), comparable across cycles',
          reportsPreviewCleaner: 'Removed the misleading per-cell % in the reports preview — they were computed over the 10 shown rows, not the full dataset',
          ltvStableWindow: 'Client LTV is now more stable: churn is computed from a fixed 12-month window regardless of the selected period',
          forecastingSimpler: '"Growth acceleration" and "Next milestone" cards hidden on /forecasting — the same information is still visible in the charts',
          cronTimeoutExtended: 'Scheduled sync timeout raised to 800s for large instances — prevents the run from being cut while paginating WHMCS data',
          mobileMenuPolish: 'Mobile web menu polish: language dropdown now opens upward (was falling off-screen) and the menu background is opaque on iOS Chrome (was rendering transparent)',
          newClientsExcludeClosed: 'The "New clients" KPI excludes clients that are already closed — only counts signups that are still customers',
          productsChurnCancellations: 'Product churn (30d) now detects cancellations with an empty terminationdate (most real-world WHMCS cancellations) — previously it always showed 0%',
          categorizationConsistency: '% of MRR categorised and the products categorised KPI are now coherent — domains count as their own category. Fixed the double "%%"',
          mrrMovementMonthEnd: 'Current-month MRR Movement uses today\'s date instead of the last day of the month — previously it pre-counted cancellations that had not happened yet',
          dbViewsSecurity: 'Database views (v_snapshot_status, user_tenants) now respect the querying user\'s RLS policies',
        },
        v270: {
        ...
        v260: {
          ...
          unlimitedTeamMembers: 'Team members are no longer capped per plan — invite as many people as you need on every tier',
        ...
        v230: {
          ...
          revenueHighlights: 'Revenue highlights block: next period projection, pending invoices, and latest payments',
          dateFilters: 'New date filters: "This month" preset and custom date range with modal picker',
          customDateRange: 'Custom date range modal for flexible period selection across all pages',
          commandPalette: '15 new quick-access entries in search (Ctrl+K): subsections, reports, settings tabs',
          settingsDeepLinks: 'Command palette links open the correct settings tab via URL params',
          syncLink: 'Sync management button added to WHMCS instances section in settings',
          htmlEntities: 'HTML entities in client/product names decoded during sync (e.g. &amp; → &)',
          syncErrorMessages: 'Improved sync error messages when WHMCS returns HTML instead of JSON',
        },
        v220: {
    ...
    forecasting: {
      ...
      accelerationDesc: {
        accelerating: 'Growth is accelerating. The percentage shows how much faster the second half of the period moved vs the first — not the total growth amount.',
        stable: 'Growth pace is steady — neither speeding up nor slowing down between the two halves of the period.',
        decelerating: 'Growth is decelerating. The percentage shows how much the pace dropped in the second half vs the first — revenue may still be growing, just more slowly.',
      },
      accelerationPct: '{{pct}}% acceleration',
      // Milestone
      ...
      howItWorksDesc: 'Projections are based on month-over-month revenue trends from all invoiced amounts (paid, unpaid and pending) over the last 6 months. Confidence increases with more consistent historical data.',
      howItWorksDescPeriod: 'Projections are based on revenue trends from all invoiced amounts (paid, unpaid and pending) within the selected period ({{period}}). Longer periods provide more data points for accurate forecasting.',
      ...
      revenueTrendTitle: 'MRR Trend',
      revenueTrendDesc: 'Historical MRR with pessimistic, baseline and optimistic projections',
    ...
    clients: {
      ...
      arpuHint: 'Average monthly recurring revenue per active client',
    ...
    connectors: {
      ...
      emailUsePlatform: 'Send from MRRlytics',
      emailUsePlatformHint: 'Use the platform SMTP — no need to configure your own credentials.',
      emailPlatformBadge: 'Via MRRlytics',
      emailHost: 'SMTP Host',
